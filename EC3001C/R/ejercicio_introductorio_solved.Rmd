...
```{r umbral, message=FALSE, warning=FALSE}
umbral = 0.5
```
...
```{r agentes, message=FALSE, warning=FALSE}
...
```{r asignacion, message=FALSE, warning=FALSE}
# Define un corte aleatorrio
asignacion_aleatoria = runif(poblacion) < umbral
# Asigna compradores
compradores = agentes[asignacion_aleatoria,]
# Asigna vendedores (tip usa la negación del vector para invertirlo)
vendedores = agentes[!asignacion_aleatoria,]
```
...
```{r periodos, message=FALSE, warning=FALSE}
...
```{r simulacion, message=FALSE, warning=FALSE}
...
for (periodo in periodos){
  ...
  comprador = sample_n(compradores, 1)
  # Extrae el precio de reserva del comprador
  precio_compra = pull(comprador, precio_compra)
  # Define la puja que el comprador está dispuesto a hacer 
  puja = runif(1) * precio_compra
   
  # Extrae un vendedor de forma aleatoria
  vendedor = sample_n(vendedores, 1)
  # Extrae el precio de reserva del vendedor
  precio_venta = pull(vendedor, precio_venta)
  
  # Almacena el precio de puja
  precios = append(precios, puja)
  # Almacena el precio de compra
  precios_compra = append(precios_compra, precio_compra)
  # Almacena el precio de venta
  precios_venta = append(precios_venta, precio_venta)
  
  # Si la puja es mayor que el precio de reserva
  if (puja >= precio_venta){
    # Agrega un TRUE al vector de trransacciones
    transacciones = append(transacciones, TRUE)
    ...
    transacciones = append(transacciones, FALSE)
  }
...
```{r hist_precios, message=FALSE, warning=FALSE}
# Utiliza los resultados de la simulación
resultados %>% 
  # Grafica los precios
  ggplot(aes(x = precios)) +
  geom_histogram(bins = 30, col = 'white', fill = 'darkred') +
  # Agrega títulos
  ggtitle('La distribución de precios se concentra por debajp de 0.5',
          subtitle = 'Conteo') +
...
```{r hist_precios_true, message=FALSE, warning=FALSE}
# Utiliza los resultados de la simulación
resultados %>% 
  # Flitra las transacciones exitosas
  filter(transacciones == TRUE) %>% 
  # Grafica los precios
  ggplot(aes(x = precios)) +
  geom_histogram(bins = 30, col = 'white', fill = 'darkred') +
  # Agrega títulos
  ggtitle('Distribución de los precios transaccionados',
          subtitle = 'Conteo') +
  #  Usa un tema predefinido
  theme_bw() +
  # Haz modificaciones
...
resultados = resultados %>% 
  ...
  mutate(beneficios = precios - precios_venta)

# Calcula las transacciones totales
transacciones_exitosas_totales = sum(pull(resultados,transacciones))

# Utiliza los resultados de la simulación
resultados %>% 
  # Flitra las transacciones exitosas
  filter(transacciones == TRUE) %>% 
  # Grafica los beneficios ordenados
  ggplot(aes(y = sort(beneficios), x = 1:transacciones_exitosas_totales)) +
  geom_col(fill = 'darkblue', alpha = 0.75) +
  xlab('Número de transacción') +
  # Agrega títulos
  ggtitle('Beneficios de los oferentes derivados de las transacciones',
          subtitle = 'Diferencia entre precios de mercado y precios de reserva') +
  #  Usa un tema predefinido
  theme_bw() +
  # Haz modificaciones
  theme(
...
- Agrega una nueva variable llamada `brecha` compuesta de la diferencia entre `precios_compra` y `precios_venta`. Grafica los resultados para todas las observaciones, ordenando de mayor a menor los beneficios. Te recomiendo usar un gráfico de puntos o un gráfico de lineas.(*Tip: Coloca la variable `brecha` en el eje de las `y`. En el eje de las `x` coloca una secuencia del `1` hasta el nùmero de observaciones*)
```{r brechas}
# Utiliza los resultados
resultados = resultados %>% 
  # Agrega una nueva variable
  mutate(brecha = precios_compra - precios_venta)


# Utiliza los resultados de la simulación
resultados %>% 
  # Grafica los beneficios ordenados
  ggplot(aes(y = desc(sort(brecha)), x = 1:nrow(resultados))) +
  geom_point(col = 'darkred', alpha = 0.75) +
  xlab('Número de periodos') +
  # Agrega títulos
  ggtitle('Brecha entre los precios de compra y venta',
          subtitle = 'Diferencia entre los precios de compra y venta') +
  #  Usa un tema predefinido
  theme_bw() +
  # Haz modificaciones
  theme(
...
resultados %>% 
  ...
  filter(transacciones == TRUE) %>% 
  # Grafica los beneficios ordenados
  ggplot(aes(y = precios)) +
  geom_point(aes(x = precios_compra, col = 'Compradores'), alpha = 0.75)+
  geom_point(aes(x = precios_venta, col = 'Vendedores'), alpha = 0.75) +
  labs(x = 'Precios de Reserva', col = 'Agentes') +
  # Agrega títulos
  ggtitle('Excedentes del Consumidor y Productor',
          subtitle = 'Precios de Mercado') +
  # Cambia los colores
...
resultados %>% 
  # Flitra las transacciones exitosas
  filter(transacciones == TRUE) %>% 
  # Ordena por brecha de mayor a menor
  arrange(desc(brecha)) %>%
  # Grafica los beneficios ordenados
  ggplot(aes(x = 1:transacciones_exitosas_totales)) +
  geom_point(aes(y = precios_compra, col = 'Compradores'), alpha = 0.75)+
  geom_point(aes(y = precios_venta, col = 'Vendedores'), alpha = 0.75) +
  labs(x = 'Precios de Reserva', col = 'Agentes') +
  # Agrega títulos
  ggtitle('Ofertas y demandas de los Zero-Intelligence Traders',
          subtitle = 'Precios de Mercado') +
  # Cambia los colores
  scale_color_manual(values = c('darkred','darkblue')) +
  #  Usa un tema predefinido
...
```{r g8, message=FALSE, warning=FALSE}
# Utiliza los resultados
resultados %>% 
  # Coloca las brechas ordenadas en 100 cajitas (bins)
  mutate(bins = ntile(desc(brecha), n = 100)) %>% 
  # Agrupa por bins
  with_groups(
    .groups = bins,
    summarise,
