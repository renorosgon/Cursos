...
```{r librerias, warning = FALSE, message = FALSE, eval = FALSE}
...
estaciones = estaciones %>% 
  ...
  mutate(LINEA = str_remove(___,'___'))
# Inspecciona el objeto
glimpse(___)
```
...
```{r nodos,  warning = FALSE, message = FALSE, eval = FALSE}
...
```{r ejes,  warning = FALSE, message = FALSE, eval = FALSE}
# Crea el objeto ejes
ejes = nodos %>% 
  # Concatena por la izquierda el objeto `estaciones`
  left_join(
    # Elimina la geometría del objeto estaciones
    st_drop_geometry(___), 
...
```{r ggraph, warning = FALSE, message = FALSE, eval = FALSE}
...
ggraph(___) + 
  ...
  geom_node_text(aes(___), repel = T) +
  # Modifica el tema
  theme_graph()
```
...
```{r graph_to_sf2, warning = FALSE, message = FALSE, eval = FALSE}
...
```{r graph_to_sf, warning = FALSE, message = FALSE, eval = FALSE}
...
ejes_red = red %>% 
  ...
  rename(___) %>% 
  # Agrega la información de la línea
...
```{r ggraph_sf, warning = FALSE, message = FALSE, eval = FALSE}
...
```{r grafo_final, warning = FALSE, message = FALSE, eval = FALSE}
# Inicia un lienzo ggplot2
ggplot() + 
  # Agrega los ejes
  geom_sf(data = ___,
